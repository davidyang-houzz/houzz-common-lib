...
ELOQUA_TOKEN_ENDPOINT = 'https://login.eloqua.com/auth/oauth2/token'
...
class Eloqua(object):
    ...
    def __init__(self, username=None, password=None, company=None,
                 test=False, redis=None, client_id=None, client_secret=None):
        """
        ...
        if not test:
            self.use_oauth = (redis is not None) and (client_id is not None) and (client_secret is not None)

            if self.use_oauth or all(arg is not None for arg in (username, password, company)):
                self.redis = redis
                self.client_id = client_id
                self.client_secret = client_secret

                url = 'https://login.eloqua.com/id'
                req = self.get(url, auth=(company + '\\' + username, password))
    ...
    def _get_access_token(self):
        KEY_PREFIX = 'epn:eloquaBatch'
        ACCESS_TOKEN_KEY = '%s:accessToken' % KEY_PREFIX
        REFRESH_TOKEN_KEY = '%s:refreshToken' % KEY_PREFIX
        access_token = self.redis.get(ACCESS_TOKEN_KEY)
        if not access_token:
            refresh_token = self.redis.get(REFRESH_TOKEN_KEY)
            if not refresh_token:
                raise Exception("ELOQUA refresh does not exist under redis key [%s]" % key)
            params = {
                "grant_type": "refresh_token",
                "refresh_token": refresh_token,
                "scope": "full",
                "redirect_uri": "https://www.houzz.com/partnerApi",
            }
            headers = {
                'Authorization': 'Basic %s' % base64.b64encode('%s:%s' % (self.client_id, self.client_secret))
            }
            resp = requests.post(ELOQUA_TOKEN_ENDPOINT, data=params, headers=headers)
            if resp.status_code != 200:
                raise Exception('failed to refresh token with response: %s' % resp)
            result = resp.json()
            access_token = result['access_token']
            self.redis.setex(name=ACCESS_TOKEN_KEY, value=access_token, time=result['expires_in'] - 10)
            TTL_ONE_YEAR = 3600 * 24 * 365
            self.redis.setex(name=REFRESH_TOKEN_KEY, value=result['refresh_token'], time=TTL_ONE_YEAR)
        return access_token

    def _add_authorization_header(self, kwargs):
        if self.use_oauth and 'auth' in kwargs:
    ...
    def get(self, *args, **kwargs):
        self._add_authorization_header(kwargs)
    ...
    def GetFields(self, entity, cdoID=0, fields=[]):
        ...
        req = self.get(url, auth=self.auth)
    ...
    def CreateDef(self, defType, entity, fields, cdoID=0, filters='',
        ...
        req = self.post(url, data=json.dumps(
    ...
    def GetSyncedData(self, defObject={}, defURI='', limit=50000, initOffset=0, retrieveLimit=1000000):
        ...
        while hasMore and len(results) < retrieveLimit:
            ...
            req = self.get(urlWhile, auth=self.auth)
    ...
    def PostSyncData(self, data, defObject={}, defURI='', maxPost=20000, syncCount=80000, timeout=1000, interval=60):
        ...
        while (hasMore):
            ...
            req = self.post(url, data=json.dumps(sendSet, ensure_ascii=False).encode(
    ...
    def PostToForm(self, data, formId=0, formHtmlName='', formName=''):
        ...
        for row in data:
            ...
            req = self.post(url, params=querystring, data=row)
            if (req.content == b'\r\n'):
                successCount += 1
            else:
                failCount += 1
    ...
    def DeleteRecord(self, entity, id, cdoID=0):
        ...
        req = self.delete(uri, auth=self.auth)
