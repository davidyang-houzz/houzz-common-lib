...
#
# Zheng: add rate limit support
# Jesse: add sanitize function for path
...
class SCPClient(object):
    ...
    """
    def __init__(self, transport, buff_size = 16384, socket_timeout = 5.0, progress = None, bandwidth = None, sanitize = _sh_quote):
        """
        ...
        @type bandwidth: int bandwidth limit in Kbit/second
        """
        ...
        assert bandwidth is None or isinstance(bandwidth, int)
    ...
    def put(self, files, remote_path = '.', 
        ...
        if self._bandwidth:
            rate_limit = ' -l %s' % self._bandwidth
        else:
            rate_limit = ''
        if recursive:
            scp_command = 'scp %s -r -t %%s' % rate_limit
        else:
            scp_command = 'scp %s -t %%s' % rate_limit
    ...
    def get(self, remote_path, local_path = '',
        ...
        self.channel = self.transport.open_session()
        self.channel.settimeout(self.socket_timeout)
        self.channel.exec_command('scp%s%s%s -f %s' % (rate_limit, rcsv, prsv, ' '.join(remote_path)))
        self._recv_all()
