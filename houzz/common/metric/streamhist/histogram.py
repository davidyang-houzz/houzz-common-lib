...
from .utils import (next_after, bin_diff, accumulate, linspace,
                   iterator_types, argmin, bin_sums, running_sums, roots)
...
class StreamHist(object):
    ...
    def insert(self, n, count):
        ...
        """
        # Some metrics are sending None as value. Don't insert.
        if n is None:
            return
    ...
    def trim(self):
        ...
        while len(self.bins) > self.maxbins:
            ...
            # This is no longer supported in sortedcontainers 2.0.x.
            # self.bins[index] += prv
            prv1 = self.bins.pop(index)
            self.bins.add(prv + prv1)
    ...
    def quantiles(self, *quantiles):
        ...
        for x in quantiles:
            if x <= 0:
                ...
                index = bisect_left(sums, target_sum) - 1  # shift left by 1 due to bisect_left
                prev_sum = sums[index]
                if len(self.bins) == 0:
                    continue
                if sums[index + 1] == target_sum:
                    # optimization: no need to compute if there is an exact match
                    qq = self.bins[index].value
                else:
                    if index <= 0:
                        bin_i = Bin(self._min, 0)  # dummy P0 bin
                        bin_i1 = self.bins[0]
                    elif index >= len(self.bins):
                        bin_i = self.bins[-1]
                        bin_i1 = Bin(self._max, 0)  # dummy PB1 bin
                    else:
                        bin_i = self.bins[index - 1]  # shift left by 1 due to dummy P0 bin
                        bin_i1 = self.bins[index]
                    qq = _compute_quantile(target_sum, bin_i, bin_i1, prev_sum)
            result.append(qq)
        return result
...
class Bin(object):
    ...
    def from_dict(cls, d):
        ...
        """
        value = d.get("mean", d.get("value", None))
        if value is None:
    ...
    def __repr__(self):
        ...
        """
        return "Bin(value=%s, count=%d)" % (self.value, self.count)
